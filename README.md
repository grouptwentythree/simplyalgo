...
- [ ] user-specified SELECT query (Use SELECT(name) on Brokerage to get all brokerage's name in the tabel) **look for brokers that can integrate with your broker**
...
- [ ] AGGREGATION query -(Use JOIN on Algotrader(aid) client(cName) and Use-build(aid), then use SELECT C.cName SUM(fee) to determine total profits made by each client) 1. **portfolio, given user id, sum all unique holdings from all the algotrader that the user owns..?** 
- [ ] NESTED AGGREGATING with GROUP BY (Use JOIN on Algotrader(aid) and Use-build(aid) tabel, then use SELECT id SUM(fee) GROUP BY id to determine total profits made for each algotrader)
